...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('formIndicadores').addEventListener('submit', enviarIndicadores);
...
async function carregarHistorico(filtro = 'todos') {
    try {
        const resposta = await fetch(
            `../../api/prefeituras/historico_completo.php?prefeitura_id=${prefeituraId}&filtro=${filtro}`
        );
        const dados = await resposta.json();
        ...
        if (dados.historico && dados.historico.length > 0) {
            ...
            lista.innerHTML = dados.historico.map(function(item) {
                return `
...
// ======================================================
// ENVIAR INDICADORES AMBIENTAIS
// ======================================================

async function enviarIndicadores(e) {
    ...
    const dados = {
        periodo: document.getElementById('indic_periodo').value,
        populacao_atendida: document.getElementById('indic_populacao_atendida').value || null,
        volume_coletado_ton: document.getElementById('indic_volume_coletado').value || null,
        volume_reciclavel_ton: document.getElementById('indic_volume_reciclavel').value || null,
        pontos_coleta_ativos: document.getElementById('indic_pontos_ativos').value || null,
        fonte: document.getElementById('indic_fonte').value || null,
        observacao: document.getElementById('indic_observacao').value || null
    };

    if (!dados.periodo) {
        mostrarModal('Erro', 'Selecione o período de referência.');
        return;
    }

    if (!dados.populacao_atendida && !dados.volume_coletado_ton && !dados.volume_reciclavel_ton && !dados.pontos_coleta_ativos) {
        mostrarModal('Erro', 'Preencha ao menos um indicador (população, volume coletado, volume reciclável ou pontos ativos).');
        return;
    }
    ...
    try {
        ...
        const resposta = await fetch('../../api/prefeituras/enviar_indicadores.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(dados)
        });
        ...
        if (resultado.sucesso) {
            mostrarModal('Sucesso!', 'Indicadores enviados para aprovação.');
            form.reset();
            carregarResumo();
            carregarHistorico();
            carregarListaIndicadores();
        } else {
            mostrarModal('Erro', resultado.mensagem || 'Erro ao enviar indicadores.');
        }
    ...
    btn.disabled = false;
    btn.innerHTML = '<i class="fas fa-paper-plane"></i> Enviar Indicadores';

}

async function carregarListaIndicadores() {
    ...
    try {
        ...
        const resposta = await fetch(
            `../../api/prefeituras/historico_completo.php?prefeitura_id=${prefeituraId}`
        );

        const dados = await resposta.json();
        const indicadores = (dados.historico || []).filter(item => item.tipo === 'indicador');

        if (indicadores.length > 0) {

            lista.innerHTML = indicadores.map(item => `
                <div class="item-lista">
                    <div class="info">
            ...
            lista.innerHTML = '<p class="sem-dados">Nenhum indicador enviado ainda.</p>';
